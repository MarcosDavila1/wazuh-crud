...
const controller = require("../controllers/getSingleUser.controller");

const getSingleUser = async (event, context, callback) => {
  ...
  try {
    const result = controller.getSingleUser(data);
    if (!result.error) {
      response = {
        statusCode: 200,
        body: result,
