...
const controller = require("../controllers/getSingleTask.controller");

const getSingleTask = async (event, context, callback) => {
  ...
  try {
    const result = controller.getSingleTask(data);
    if (!result.error) {
      response = {
        statusCode: 200,
        body: result,
